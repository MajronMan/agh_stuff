...
int main() {
    ...
    qid = open_queue(SERVER_NAME, 'r');

    printf("----------------\n|SERVER IS UP!!|\n----------------\n");
    while (!doexit || not_empty(qid)) {
...
void hello(char *q_name) {
    msg message;
    mqd_t qid;
    CHECK_NEGATIVE_ONE("hello", qid = open_queue(q_name, 'w'), "Cannot open client queue")
...
void remove_queue() {
    CHECK_NEGATIVE_ONE("main", mq_close(qid), "Cannot close queue")
    CHECK_NEGATIVE_ONE("main", mq_unlink(SERVER_NAME), "Cannot delete queue")
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i]) {
            SEND(clients[i], &shutdownMessage);
            CHECK_NEGATIVE_ONE("remove_queue", mq_close(clients[i]), "Cannot close client queue")
        }
    }
}